...
public class DecryptionFilter implements Filter {
	...
	private HttpServletRequest wrap(String version, HttpServletRequest request) throws Exception {
		...
		KeysetHandle privateHandle = privateHandle();
		HybridDecrypt decrypt = HybridDecryptFactory.getPrimitive(privateHandle);
		byte[] plaintext = decrypt.decrypt(bodyBytes, version.getBytes(UTF_8));

		return new DecryptedWrapper(request, new ByteArrayInputStream(plaintext));
	}
	...
	private static KeysetHandle privateHandle() {
		try {
			return CleartextKeysetHandle.read(JsonKeysetReader.withString("{\n" +
					"    \"primaryKeyId\": 725480028,\n" +
					"    \"key\": [{\n" +
					"        \"keyData\": {\n" +
					"            \"typeUrl\": \"type.googleapis.com/google.crypto.tink.EciesAeadHkdfPrivateKey\",\n" +
					"            \"keyMaterialType\": \"ASYMMETRIC_PRIVATE\",\n" +
					"            \"value\": \"EosBEkQKBAgCEAMSOhI4CjB0eXBlLmdvb2dsZWFwaXMuY29tL2dvb2dsZS5jcnlwdG8udGluay5BZXNHY21LZXkSAhAQGAEYARogbGBT2BLZybQCx6rqDZhZgKOAXRWg62qIP8vknPBtujUiIQDFjAbXp+cUzQSjLflhU0UDDmhbO9WaWXgS3Rk0o+4ARBogIg019SSwu3WP7sCi3o4ZrP+ufZ356sFjOaS+IPxtBkk=\"\n" +
					"        },\n" +
